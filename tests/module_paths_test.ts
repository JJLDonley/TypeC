import { canonicalModulePath, isCHeaderPath, resolveModuleImportPath } from "module/paths.ts";
...
Deno.test("resolves relative module imports", () => {
  ...
  assertText(
    resolveModuleImportPath("/project/src/main.tc", "./math.tc", config),
    "/project/src/math.tc",
  );
  assertText(
    resolveModuleImportPath("/project/src/main.tc", "../math.tc", config),
...
Deno.test("resolves project dependency module imports", () => {
  const config = projectConfig("/project", [["basic/math", "vendor/math.tc"], [
    "abs/math",
    "/opt/math.tc",
  ]]);
  assertText(
    resolveModuleImportPath("/project/src/main.tc", "basic/math", config),
    "/project/vendor/math.tc",
  );
...
Deno.test("resolves std module imports", () => {
  ...
  assertSame(
    resolveModuleImportPath("/project/src/main.tc", "basic/math", config).endsWith("/std/math.tc"),
    true,
  );
});
...
Deno.test("reports missing canonical module paths", async () => {
  try {
    ...
    if (
      ...
      error.diagnostics[0]?.message === "Module not found '/definitely/missing/typec/module.tc'"
    ) return;
    throw error;
  }
