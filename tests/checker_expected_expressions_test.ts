...
Deno.test("checks expected primitive expressions", () => {
  ...
  const stringResult = checkExpectedExpression(stringLiteral("hi"), "u8*", aliases(), resolveExpected);
...
Deno.test("checks expected aggregate expressions", () => {
  ...
  const recordResult = checkExpectedExpression(recordLiteral([recordField("x")]), "Vec2", map, resolveExpected);
  const arrayResult = checkExpectedExpression(arrayLiteral([integer("1")]), "i32[]", aliases(), resolveExpected);
...
function recordLiteral(fields: Extract<Expression, { kind: "RecordLiteralExpr" }>["fields"]): Expression {
  return { kind: "RecordLiteralExpr", fields, span };
}

function recordField(name: Str): Extract<Expression, { kind: "RecordLiteralExpr" }>["fields"][usize] {
