...
export function checkRecordLiteralFields(expr: RecordLiteralExpr, record: RecordTypeRef, expected: TypeName, resolveType: TypeResolver): Diagnostic[] {
  const diagnostics: Diagnostic[] = [];
  const seen = new Set<Str>();
  for (const field of expr.fields) diagnostics.push(...checkRecordLiteralField(field, record, expected, seen, resolveType));
  diagnostics.push(...checkRecordLiteralMissingFields(expr, record, expected, seen));
  return diagnostics;
}

function checkRecordLiteralField(field: RecordLiteralExpr["fields"][usize], record: RecordTypeRef, expected: TypeName, seen: Set<Str>, resolveType: TypeResolver): Diagnostic[] {
  ...
  if (!isAssignable(actual, expectedType)) diagnostics.push({ message: `Field '${field.name}' type '${actual}' is not assignable to '${expectedType}'`, span: field.span });
  return diagnostics;
}
