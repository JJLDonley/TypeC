...
export function checkIndexExpression(expr: IndexExpr, operandType: TypeName, resolveType: TypeResolver): IndexExpressionCheck {
  const array = parseArrayType(operandType);
  if (!array) return { diagnostics: [{ message: `Cannot index non-array type '${operandType}'`, span: expr.span }], type: "<error>" };
