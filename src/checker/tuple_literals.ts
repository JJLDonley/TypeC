...
export function checkTupleLiteralExpression(
  ...
  for (let index: usize = 0; index < expr.elements.length && index < tuple.elements.length; index++) {
...
function checkTupleLength(expr: ArrayLiteralExpr, elements: TypeName[]): Diagnostic[] {
  ...
  return [{
    message: `Tuple literal length ${expr.elements.length} does not match expected length ${elements.length}`,
    span: expr.span,
  }];
}
