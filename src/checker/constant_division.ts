...
export function checkConstantIntegerDivision(
  ...
  if (
    expr.kind === "UnaryExpr" || expr.kind === "NonNullAssertExpr" ||
    expr.kind === "OptionalFieldAccessExpr"
  ) {
    return checkConstantIntegerDivision(expr.operand, constants);
  }
  if (expr.kind === "OptionalIndexExpr") {
    return [
      ...checkConstantIntegerDivision(expr.operand, constants),
      ...checkConstantIntegerDivision(expr.index, constants),
    ];
  }
  if (expr.kind === "OptionalMethodCallExpr") {
    return [
      ...checkConstantIntegerDivision(expr.receiver, constants),
      ...expr.args.flatMap((arg) => checkConstantIntegerDivision(arg, constants)),
    ];
  }
