...
function parseFieldAccess(parser: PostfixExpressionParser, operand: CastExpression): CastExpression {
  const field = parseFieldAccessName(parser);
  return { kind: "FieldAccessExpr", operand, field: field.text, span: span(operand.span.start, field.span.end) };
}

function parseIndexAccess(parser: PostfixExpressionParser, operand: CastExpression): CastExpression {
  const close = parseIndexClose(parser);
  return { kind: "IndexExpr", operand, index: close.index, span: span(operand.span.start, close.end.span.end) };
}

function parsePointerPostfix(parser: PostfixExpressionParser, operand: CastExpression): CastExpression {
  const op = parser.advance();
  return { kind: "PostfixPointerExpr", operator: op.text as ".*" | ".&", operand, span: span(operand.span.start, op.span.end) };
}
...
function isPostfixStart(parser: PostfixExpressionParser): b8 {
  return parser.checkText(".*") || parser.checkText(".&") || parser.checkText(".") || parser.checkText("[");
}
